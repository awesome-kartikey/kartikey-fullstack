...
JWT is a token the server gives you after login. You send this token with your requests so the server knows who you are. The server does not store session data; it only checks the token.

---

## Introduction

**Audience:** Beginners learning authentication.
**Goal:** Understand what JWT is and how it is used in login flows.
**What’s inside:**
...
JWT keeps things stateless, so the server only verifies the token.
This information is valid for most modern web apps and APIs.

---

## Mindmap

![jwt-mindmap](jwt-mindmap.png)
